...
class LTXVideoCausalConv3d(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, mask: torch.Tensor | None = None) -> torch.Tensor:
        ...
        # NEW: clamp masked suffix to last valid frame before any temporal mixing
        if mask is not None:
            hidden_states = _temporal_clamp_tail_to_last_valid(hidden_states, mask)
        ...
        # NEW: ensure masked outputs remain zero (exactly like decoding then padding with zeros)
        if mask is not None:
            hidden_states = _temporal_zero_tail(hidden_states, mask)
        return hidden_states
...
class LTXVideoResnetBlock3d(nn.Module):
    ...
    def forward(
        ...
        mask: torch.Tensor | None = None,  # NEW
    ) -> torch.Tensor:
        ...
        hidden_states = self.conv1(hidden_states, mask=mask)  # NEW
        ...
        hidden_states = self.conv2(hidden_states, mask=mask)  # NEW
        ...
        if self.conv_shortcut is not None:
            inputs = self.conv_shortcut(inputs, mask=mask)  # NEW

        hidden_states = hidden_states + inputs

        # NEW: keep masked tail strictly zero after the residual add too
        if mask is not None:
            hidden_states = _temporal_zero_tail(hidden_states, mask)
...
class LTXVideoUpsampler3d(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, mask: torch.Tensor | None = None) -> torch.Tensor:
        ...
        # --- mask-aware upsample conv
        hidden_states = self.conv(hidden_states, mask=mask)

        hidden_states = hidden_states.reshape(
            batch_size, -1, self.stride[0], self.stride[1], self.stride[2], num_frames, height, width
        )
        hidden_states = (
            hidden_states.permute(0, 1, 5, 2, 6, 3, 7, 4).flatten(6, 7).flatten(4, 5).flatten(2, 3)
        )
        hidden_states = hidden_states[:, :, self.stride[0] - 1 :]

        # --- expand mask to new T and zero tail
        if mask is not None:
            m = mask.repeat_interleave(self.stride[0], dim=2)
            m = m[:, :, self.stride[0] - 1 :]
            hidden_states = _temporal_zero_tail(hidden_states, m)

        if self.residual:
            if mask is not None:
                residual = _temporal_zero_tail(residual, m)
            hidden_states = hidden_states + residual
            if mask is not None:
                hidden_states = _temporal_zero_tail(hidden_states, m)
        ...
        for resnet in self.resnets:
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                hidden_states = self._gradient_checkpointing_func(
                    resnet, hidden_states, temb, generator, mask
                )
            else:
                hidden_states = resnet(hidden_states, temb, generator, mask=mask)  # NEW
        return hidden_states
...
class LTXVideoUpBlock3d(nn.Module):
    ...
    def forward(
        ...
        if self.conv_in is not None:
            hidden_states = self.conv_in(hidden_states, temb, generator, mask=mask)
        ...
        if self.upsamplers is not None:
            for upsampler in self.upsamplers:
                hidden_states = upsampler(hidden_states, mask=mask)  # ← pass mask in
                mask = _upsample_mask_temporally(mask, upsampler.stride[0])
                if mask is not None:
                    hidden_states = _temporal_zero_tail(hidden_states, mask)
        ...
        for resnet in self.resnets:
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                ...
                hidden_states = resnet(hidden_states, temb, generator, mask=mask)
...
class LTXVideoEncoder3d(nn.Module):
    ...
    def __init__(
        ...
        for i in range(num_block_out_channels):
            ...
            if not is_ltx_095:
                output_channel = (
                    block_out_channels[i + 1] if i + 1 < num_block_out_channels else block_out_channels[i]
                )
            else:
...
class LTXVideoDecoder3d(nn.Module):
    ...
    def forward(
        ...
        hidden_states: torch.Tensor,
        temb: torch.Tensor | None = None,
        latent_mask: torch.Tensor | None = None,  # NEW
    ) -> torch.Tensor:
        mask = _normalize_mask(latent_mask, hidden_states)  # (B,1,Tz,1,1) or None

        # conv in & mid
        hidden_states = self.conv_in(hidden_states, mask=mask)
        hidden_states = self.mid_block(hidden_states, temb, mask=mask)

        # up blocks (mask gets expanded inside up block)
        for up_block in self.up_blocks:
            hidden_states = up_block(hidden_states, temb, mask=mask)
            # NOTE: `mask` is expanded inside the block; keep carrying it as the same object
            if mask is not None and getattr(up_block, "upsamplers", None) is not None:
                for upsampler in up_block.upsamplers:
                    mask = _upsample_mask_temporally(mask, upsampler.stride[0])

        # norm + optional temporal conditioning
        hidden_states = self.norm_out(hidden_states.movedim(1, -1)).movedim(-1, 1)
        if self.time_embedder is not None:
            temb = self.time_embedder(
                timestep=temb.flatten(),
        ...
        hidden_states = self.conv_act(hidden_states)
        hidden_states = self.conv_out(hidden_states, mask=mask)

        p = self.patch_size
        p_t = self.patch_size_t

        b, c, tz, h, w = hidden_states.shape
        hidden_states = hidden_states.reshape(b, -1, p_t, p, p, tz, h, w)

        # CORRECT: put Tz first, then p_t, then H, W (matching the encoder’s patching order)
        hidden_states = (
            hidden_states.permute(0, 1, 5, 2, 6, 4, 7, 3)  # (B, C', Tz, p_t, H, p, W, p)
            .flatten(6, 7)  # W_out = W * p
            .flatten(4, 5)  # H_out = H * p
            .flatten(2, 3)  # T_out = Tz * p_t
        )

        # NEW: keep decoded padding strictly zero too
        if mask is not None and p_t > 1:
            sample_mask = mask.repeat_interleave(p_t, dim=2)  # (B,1,T,1,1)
            hidden_states = _temporal_zero_tail(hidden_states, sample_mask)
        elif mask is not None:
            hidden_states = _temporal_zero_tail(hidden_states, mask)  # same T if p_t==1

        return hidden_states


def _normalize_mask(mask, like: torch.Tensor) -> torch.Tensor:
    """
    mask: (B, Tz) or (B,1,Tz,1,1) -> returns bool (B,1,Tz,1,1) on same device as `like`.
    ...
    if mask.dim() == 2:
        ...
        raise ValueError("mask must be (B, T) or (B,1,T,1,1)")
    return mask.to(like.device) > 0.5


def _temporal_zero_tail(x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    if mask is None:
        return x
    return x * mask  # broadcast to (B,C,T,H,W)


def _temporal_clamp_tail_to_last_valid(x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    """
    Replace the *masked* suffix frames with the last valid frame (per-sample), across all C,H,W.
    This is applied only to the *inputs* of a time-mixing op, and we zero tail *after* the op.
    """
    if mask is None:
        return x
    B, C, T, H, W = x.shape
    m = mask.squeeze(1).squeeze(-1).squeeze(-1)  # (B, T)
    # number of valid frames per sample
    valid_counts = m.sum(dim=1)  # (B,)
    last_idx = (valid_counts - 1).clamp(min=0).to(torch.long)  # (B,)

    idx = last_idx.view(B, 1, 1, 1, 1).expand(B, C, T, H, W)  # gather needs full shape
    last = torch.gather(x, 2, idx)  # (B,C,T,H,W), each time step = last valid

    full_mask = mask.expand_as(x)
    return torch.where(full_mask, x, last)


def _upsample_mask_temporally(mask: torch.Tensor, stride_t: int) -> torch.Tensor:
    """
    Matches LTXVideoUpsampler3d temporal behavior (repeat then crop [:, :, stride_t-1:]).
    """
    if mask is None:
        return None
    mask = mask.repeat_interleave(stride_t, dim=2)
    return mask[:, :, stride_t - 1 :]
...
class AutoencoderKLLTXVideo(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def _encode(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.normalize_latents:
            mu = self.latents_mean
            sigma = self.latents_std
            mu = mu[None, :, None, None, None]
            sigma = sigma[None, :, None, None, None]

            enc[:, : enc.shape[1] // 2] -= mu
            enc[:, : enc.shape[1] // 2] /= sigma
    ...
    def _decode(
        ...
        dec = self.decoder(z, temb, latent_mask=latent_mask)
        if not return_dict:
            return (dec,)
        return DecoderOutput(sample=dec)
    ...
    def decode(
        ...
        if temb is None and getattr(self.config, "timestep_conditioning", False):
            ...
            if latent_mask is not None:
                ...
                z = z_noisy * m + z * (1 - m)  # noisy where real (mask=1), original where fake (mask=0)
            else:
                z = z_noisy

        if self.use_slicing and z.shape[0] > 1:
            z_slices = list(z.split(1))
            ...
            decoded = self._decode(z, temb, latent_mask=latent_mask).sample
        if not return_dict:
            return (decoded,)
        ...
        for i, row in enumerate(rows):
            ...
            for j, tile in enumerate(row):
                ...
                if j > 0:
                    tile = self.blend_h(row[j - 1], tile, blend_width)
                result_row.append(
                    tile[:, :, :, : self.tile_sample_stride_height, : self.tile_sample_stride_width]
                )
            result_rows.append(torch.cat(result_row, dim=4))
    ...
    def _temporal_tiled_decode(
        ...
        for i in range(0, num_frames, tile_latent_stride_num_frames):
            ...
            if self.use_tiling and (
                tile.shape[-1] > tile_latent_min_width or tile.shape[-2] > tile_latent_min_height
            ):
