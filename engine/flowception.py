...
from modules.flowception.sampling import vanilla_sample_flowception, vanilla_sample_flowception_prescribed
from modules.flowception.schedulers import get_kappa_scheduler
from modules.sampling.apg import APGGuider


class Flowception(Trainer):
    """Flowception trainer for image-to-video generation.

    Implements the Flowception temporal interpolation algorithm: given anchor
    frames, iteratively inserts new frames between existing ones using a
    ...
    Key methods:
        get_flowception_loss: Computes the combined velocity + insertion loss.
        sample_tau_tau_max: Samples interpolation timestamps via rejection sampling.
        sampling_step: Generates video frames given anchor images.
        generate_samples: Full sampling pipeline with video saving.
    """
    ...
    def image_loss(self, model, batch):
        ...
        # use the provided anchor frame if available; fall back to first frame of img
    ...
    def sample_tau_tau_max(
        ...
        while need.any() and tries < max_tries:
            ...
            if global_dist == "lognorm":
                ...
                u = torch.sigmoid(e0).clamp(eps, 1.0 - eps)  # logit-normal (0,1)
            ...
            # update `need` (rows that still have no valid site)
            need_idx = idx[~ok_bad]
            need[need_idx] = False  # wait, this looks reversed? careful!
    ...
    def get_flowception_loss(self, Y1, M1, vid_lengths, cond_t, context_frames):
        ...
        # --- i2v: only the 1st frame is context ---
        ...
        M_t[:, 0] = 1
        Y_t[:, 0] = Y1[:, 0]  # keep first frame clean.
        ...
        if self.mask_first_frame:
            # generate no image mask.
            mask = torch.rand(X_t_m.shape[0], device=self.accelerator.device) > self.conditioner.cfg_p
            X_t_m[:, :1] *= mask[:, None, None, None, None]
            context_full[:, :1] *= mask[:, None, None, None, None]

        X_t_m[:, 0] = X_t_m[:, 0] * 0.9 + 0.1 * torch.randn_like(X_t_m[:, 0])
        context_full[:, 0] = context_full[:, 0] * 0.9 + 0.1 * torch.randn_like(context_full[:, 0])
        ...
        valid_flow_mask = aligned_flow_site_mask & (tau_left_aligned < 1.0)  # [B, L] bool
        valid_flow_mask[:, 0] = False  # drop context frame from flow loss
    ...
    def image_video_train_step(self, model, batch, extra_batch=None):
        ...
        with self.accelerator.autocast():
            ...
            # decorrelate image mask and text mask.
            if self.mask_first_frame:
        ...
        with self.accelerator.autocast():
            for _ in range(int(np.ceil(grid_size / (self.accelerator.num_processes * self.batch_size)))):
                ...
                if self.insertion_rule == "learned":
                    Y_t, M_t, Y_t_l, M_t_l, ins_timemap, expected_l = vanilla_sample_flowception(
    ...
    def sample_text_to_image_flow_only(
        ...
        """
        Text-to-image sampling that:
        • starts from ONE random latent frame
        • runs flow denoising updates only (ignores lambda/insertions)
        • returns latents as [B, C, Tz(=1), H, W] and mask [B, 1] (all True)

        Args:
            captions: list of prompt strings (len == batch size)
            idx:      dataset indices used only to probe VAE latent shape cheaply
            num_steps: flow steps
            s_text:   CFG scale (defaults to self.cfg_scale if None)
            keep_emb: if True, copy cond embeddings into uc slot 1 (your legacy behavior)
            use_cfg:  apply classifier-free guidance

        Returns:
            Yg: [B, C, 1, H, W]  latents
            M_t: [B, 1]         mask (all True)
        """
        device = self.accelerator.device
        B = len(captions)
        s_text_eff = self.cfg_scale if s_text is None else s_text

        # --- build conditioning (no image/context for T2I) ---
        ...
        # --- probe VAE once to get latent spatial shape ---
        # we fetch a single-frame clip for each sample; only used to infer C,H,W of z
        ds = self.val_dataloader.dataset
        probe = [ds.__getitem__(i).pixel_values for i in idx]
        probe = torch.stack(probe).to(device, non_blocking=True)  # [B,C,T,H,W] or [B,C,H,W]
        if probe.ndim == 4:
            probe = probe[:, :, None]
        probe = probe[:, :, :1]  # ensure 1 frame
        ...
        # --- init single-frame latent & mask ---
        Y_t = torch.randn(B, 1, C_z, H_z, W_z, device=device, dtype=lat.dtype)
        M_t = torch.ones(B, 1, dtype=torch.bool, device=device)

        h_grid = scale_snr(
            torch.linspace(0, 1, num_steps + 1, device=device, dtype=lat.dtype), sigma_scale=self.sigma_scale
        )

        # --- flow-only loop (ignore insertion head) ---
        for s in range(num_steps):
            t = h_grid[s].expand(B, 1)
            dt = (h_grid[s + 1] - h_grid[s]).expand(B, 1)
            ...
            Y_t = Y_t + dt[..., None, None, None] * vel

        # return in [B, C, T, H, W] layout plus mask
